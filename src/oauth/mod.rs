...
impl Authenticator {
    ...
    pub fn new(config: &AppConfig) -> Result<Self> {
        let oauth2_client = utils::build_oauth2_client_from_config(config)?;
    ...
    pub async fn start_auth_flow(&self, auth_params: AuthFlow) -> Result<String> {
    ...
    pub async fn exchange_code_for_token(&self, request: TokenRequest) -> Result<String> {
    ...
    pub fn encrypt_token(&self, token: String) -> Result<String> {
    ...
    pub fn decrypt_token(&self, ciphertext: String) -> Result<String> {
    ...
    pub async fn generate_single_use_token(
        &self,
        token: String,
        url: String,
    ...
    /// single possible use.
    pub async fn verify_single_use_token(&self, token: &str, url: &str) -> Result<SingleUseToken> {
        let token = self
            .download_tokens
            .write()
            .await
...
mod tests {
    ...
    fn build_test_authenticator() -> Authenticator {
        Authenticator::new(&AppConfig {
            client_id: "a".into(),
    ...
    async fn single_use_token_missing() {
        let auth = build_test_authenticator();
        let token = auth.verify_single_use_token("token", "url").await;
    ...
    async fn single_use_token_bad_url() {
        ...
        let token = auth
            .verify_single_use_token(&encrypted_token.unwrap(), "bad url")
            .await;
